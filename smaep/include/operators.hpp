...
template <typename TValue> auto create_parser_config() {
  ...
  config->add_operator(
      "^", order(4), [](const TValue l, const TValue r) { return pow(l, r); });
  ...
  config->add_function("exp", order(1),
                       [](const TValue x) { return std::exp(x); });

  config->add_function("sin", order(1),
                       [](const TValue x) { return std::sin(x); });
  config->add_function("cos", order(1),
                       [](const TValue x) { return std::cos(x); });
  config->add_function("tan", order(1),
                       [](const TValue x) { return std::tan(x); });
