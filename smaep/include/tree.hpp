...
struct inode {
  ...
  virtual std::string get_X() = 0;

  virtual void to_dot(std::ostream& dot, uint32_t inc = 0) {
    dot << std::string(2 * inc, ' ') <<  " └ " << std::left << std::setw(30 - 2 * inc) << get_X() << "  - 0x"
        << this
        //<< "  " << typeid(this).name()
        << std::endl;
  }
...
template <typename TValue>
struct const_node : inode<TValue> {
  TValue data;
  ...
  std::string get_X() override { return std::to_string(data); }
};

template <typename TValue>
struct var_node : inode<TValue> {
  std::string selector;
  ...
  std::string get_X() override { return "\"" + selector + "\""; }
};

template <typename TValue>
struct i_function_node : inode<TValue> {
  virtual size_t N() = 0;
};
...
std::unique_ptr<function_node<TOut, TIn...>> make_node(
    const operation<TOut, TIn...>& op) {
